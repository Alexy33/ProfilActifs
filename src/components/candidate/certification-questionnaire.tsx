    ...
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-5xl items-center px-5 py-12 md:px-10">
        <section className="w-full px-2 py-8 text-center md:px-8 md:py-12">
          ...
          <p className="mt-6 font-mono text-xs font-semibold uppercase tracking-[0.16em] text-[#41556E]">Résultat de certification</p>
          <p className="mt-5 text-7xl font-extrabold tracking-tight text-[#1B3A6B]">{result.score}<span className="text-2xl text-[#41556E]"> / 100</span></p>
          <h1 className="mt-6 text-3xl font-extrabold uppercase text-[#22334D] md:text-4xl">{result.passed ? "Certification obtenue !" : "Continuez vos efforts"}</h1>
          <p className="mx-auto mt-4 max-w-xl text-base leading-relaxed text-[#41556E]">{result.passed ? "Votre badge JEB est maintenant visible sur votre profil public." : `Le seuil est de ${result.threshold}/100. Vous pouvez repasser le questionnaire sans délai.`}</p>
          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <Link href="/candidate" className="inline-flex h-12 items-center justify-center rounded-2xl bg-[#2d3748] px-6 font-semibold text-white hover:bg-[#1E293B]">Retour à mon espace</Link>
            <button type="button" onClick={restart} disabled={busy} className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl border border-[#1B3A6B]/25 px-6 font-semibold text-[#22334D] hover:bg-white disabled:opacity-60"><RotateCcw className="size-4" /> Repasser</button>
          </div>
        </section>
      </main>
    );
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-5 pb-20 pt-8 md:px-10 md:pt-12">
      ...
      <header className="mt-8 flex items-end justify-between gap-5">
        <div><p className="font-mono text-xs font-semibold uppercase tracking-wider text-[#41556E]">Certification professionnelle JEB</p><h1 className="mt-3 text-3xl font-extrabold uppercase text-[#22334D] md:text-5xl">Valorisez vos <span className="text-[#1B3A6B]">aptitudes.</span></h1></div>
        <p className="shrink-0 font-mono text-xs font-semibold text-[#41556E]">{index + 1} / {questions.length}</p>
      </header>
      ...
      <section className="mt-8 border-t border-[#1B3A6B]/15 pt-8">
        ...
        <div className="mt-8 flex items-center justify-between gap-4 border-t border-[#1B3A6B]/15 pt-6">
          <button type="button" onClick={() => setIndex((value) => Math.max(0, value - 1))} disabled={index === 0 || busy} className="inline-flex h-11 items-center gap-2 rounded-xl px-4 text-sm font-semibold text-[#41556E] hover:bg-white disabled:opacity-40"><ArrowLeft className="size-4" /> Précédent</button>
          {index < questions.length - 1 ? <button type="button" onClick={() => setIndex((value) => value + 1)} disabled={selected === undefined || busy} className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#2d3748] px-5 text-sm font-semibold text-white hover:bg-[#1E293B] disabled:opacity-40">Suivant <ArrowRight className="size-4" /></button> : <button type="button" onClick={submit} disabled={selected === undefined || busy} className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#2d3748] px-5 text-sm font-semibold text-white hover:bg-[#1E293B] disabled:opacity-40">{busy && <Loader2 className="size-4 animate-spin" />} Valider le questionnaire</button>}
        </div>
      </section>
    </main>
  );

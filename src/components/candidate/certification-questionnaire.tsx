    ...
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-5xl items-center px-5 py-12 md:px-10">
        <section className="w-full px-2 py-8 text-center md:px-8 md:py-12">
          ...
          <p className="mt-6 font-mono text-xs font-semibold uppercase tracking-[0.16em] text-[#566274]">Résultat de certification</p>
          <p className="mt-5 text-7xl font-extrabold tracking-tight text-[#1B3A6B]">{result.score}<span className="text-2xl text-[#566274]"> / 100</span></p>
          ...
          <p className="mx-auto mt-4 max-w-xl text-base leading-relaxed text-[#566274]">{result.passed ? "Votre badge JEB est maintenant visible sur votre profil public." : `Le seuil est de ${result.threshold}/100. Vous pouvez repasser le questionnaire sans délai.`}</p>
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-5 pb-20 pt-8 md:px-10 md:pt-12">
      ...
      <header className="mt-8 flex items-end justify-between gap-5">
        <div><p className="font-mono text-xs font-semibold uppercase tracking-wider text-[#566274]">Certification professionnelle JEB</p><h1 className="mt-3 text-3xl font-extrabold uppercase text-[#2d3748] md:text-5xl">Valorisez vos <span className="text-[#1B3A6B]">aptitudes.</span></h1></div>
        <p className="shrink-0 font-mono text-xs font-semibold text-[#566274]">{index + 1} / {questions.length}</p>
      </header>

...
export function CertificationQuestionnaire({ initialQuestions, initialState }: { initialQuestions: Question[]; initialState: State }) {
  ...
  if (questions.length === 0) {
    return <main className="mx-auto flex min-h-screen max-w-3xl flex-col items-center justify-center px-6 text-center"><FileQuestion className="size-10 text-[#5980a6]" /><h1 className="mt-5 text-2xl font-bold text-[#2d3748]">Questionnaire indisponible</h1><p className="mt-3 text-[#718096]">Aucune question n’est configurée. Demandez à un administrateur d’ajouter le questionnaire.</p><Link href="/candidate" className="mt-6 font-semibold text-[#5980a6]">Retour à mon espace</Link></main>;
  }
    ...
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-5xl items-center px-5 py-12 md:px-10">
        <section className="w-full px-2 py-8 text-center md:px-8 md:py-12">
          ...
          <p className="mt-5 text-7xl font-extrabold tracking-tight text-[#5980a6]">{result.score}<span className="text-2xl text-[#718096]"> / 100</span></p>
          ...
          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <Link href="/candidate" className="inline-flex h-12 items-center justify-center rounded-2xl bg-[#5980a6] px-6 font-semibold text-white hover:bg-[#416180]">Retour à mon espace</Link>
            <button type="button" onClick={restart} disabled={busy} className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl border border-[#5980a6]/25 px-6 font-semibold text-[#2d3748] hover:bg-white disabled:opacity-60"><RotateCcw className="size-4" /> Repasser</button>
          </div>
        </section>
      </main>
    );
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-5 pb-20 pt-8 md:px-10 md:pt-12">
      <Link href="/candidate" className="inline-flex items-center gap-2 text-sm font-semibold text-[#5980a6] hover:text-[#416180]"><ArrowLeft className="size-4" /> Retour à mon espace</Link>
      <header className="mt-8 flex items-end justify-between gap-5">
        <div><p className="font-mono text-xs font-semibold uppercase tracking-wider text-[#718096]">Certification professionnelle JEB</p><h1 className="mt-3 text-3xl font-extrabold uppercase text-[#2d3748] md:text-5xl">Valorisez vos <span className="text-[#5980a6]">aptitudes.</span></h1></div>
        <p className="shrink-0 font-mono text-xs font-semibold text-[#718096]">{index + 1} / {questions.length}</p>
      </header>

      <div className="mt-7 h-2 overflow-hidden rounded-full bg-[#d4dee9]"><div className="h-full rounded-full bg-[#5980a6] transition-[width] duration-500" style={{ width: `${progress}%` }} /></div>

      <section className="mt-8 border-t border-[#5980a6]/15 pt-8">
        <div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-[#d6ebff] font-mono text-sm font-bold text-[#2c455d]">{String(index + 1).padStart(2, "0")}</span><span className="inline-flex items-center gap-2 text-xs text-[#718096]"><Save className="size-3.5" /> Réponse sauvegardée automatiquement</span></div>
        <h2 className="mt-7 max-w-3xl text-2xl font-bold leading-snug text-[#2d3748] md:text-3xl">{question.text}</h2>
        <div className="mt-8 grid gap-3">
          {question.options.map((option, optionIndex) => {
            const active = selected === optionIndex;
            return <button key={option.id} type="button" onClick={() => void choose(optionIndex)} disabled={busy} className={`flex min-h-16 items-center gap-4 rounded-2xl border px-5 py-4 text-left text-sm font-semibold transition-colors md:text-base ${active ? "border-[#5980a6] bg-[#d6ebff] text-[#2c455d]" : "border-[#5980a6]/15 bg-white text-[#4a5568] hover:border-[#5980a6]/50 hover:bg-[#f8fbff]"}`}><span className={`flex size-6 shrink-0 items-center justify-center rounded-full border ${active ? "border-[#5980a6] bg-[#5980a6] text-white" : "border-[#5980a6]/25"}`}>{active && <Check className="size-3.5" />}</span>{option.label}</button>;
          })}
        </div>
        {error && <p role="alert" className="mt-5 rounded-xl bg-[#ffe8ef] px-4 py-3 text-sm text-[#8a3f5b]">{error}</p>}
        <div className="mt-8 flex items-center justify-between gap-4 border-t border-[#5980a6]/15 pt-6">
          ...
          {index < questions.length - 1 ? <button type="button" onClick={() => setIndex((value) => value + 1)} disabled={selected === undefined || busy} className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#5980a6] px-5 text-sm font-semibold text-white hover:bg-[#416180] disabled:opacity-40">Suivant <ArrowRight className="size-4" /></button> : <button type="button" onClick={submit} disabled={selected === undefined || busy} className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#5980a6] px-5 text-sm font-semibold text-white hover:bg-[#416180] disabled:opacity-40">{busy && <Loader2 className="size-4 animate-spin" />} Valider le questionnaire</button>}
        </div>
      </section>
    </main>
  );

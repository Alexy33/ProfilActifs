...
export function AdminDashboard() {
  ...
  return <main className="mx-auto w-full max-w-[1480px] px-5 pb-24 pt-8 md:px-10 md:pt-12 lg:px-6">
    <header className="border-b border-[#1B3A6B]/15 pb-7"><p className="font-mono text-xs font-semibold uppercase tracking-wider text-[#566274]">Administration du dispositif</p><h1 className="mt-3 text-4xl font-extrabold uppercase text-[#2d3748] md:text-5xl">Pilotez la <span className="text-[#1B3A6B]">plateforme.</span></h1></header>
    {message && <p role="status" className="mt-5 rounded-xl bg-[#D1DEF0] px-4 py-3 text-sm text-[#1B2D3E]">{message}</p>}
    <section className="mt-6 grid grid-cols-2 gap-3 xl:grid-cols-4">{cards.map(({ label, value, icon: Icon, tone }) => <article key={label} className="flex items-center gap-4 rounded-2xl border border-[#1B3A6B]/15 bg-white p-5"><span className={`flex size-10 items-center justify-center rounded-xl ${tone}`}><Icon className="size-5" /></span><div><p className="text-2xl font-extrabold text-[#2d3748]">{value}</p><p className="text-xs text-[#566274]">{label}</p></div></article>)}</section>
    {loading ? <div className="flex justify-center py-24"><Loader2 className="size-7 animate-spin text-[#1B3A6B]" /></div> : <>
      <nav aria-label="Sections de l&apos;administration" className="mt-7 flex flex-wrap gap-2 border-b border-[#1B3A6B]/15 pb-3">
        {TABS.map(({ id, label, icon: Icon }) => {
          ...
          return <button key={id} type="button" onClick={() => setTab(id)} aria-current={tab === id ? "page" : undefined} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold ${tab === id ? "bg-[#1B3A6B] text-white" : "bg-white text-[#4a5568] hover:bg-[#D1DEF0]"}`}>
      ...
      <section className="rounded-3xl bg-[#ebf0f7] p-6 shadow-[10px_10px_20px_#c5d1e0,-10px_-10px_20px_#ffffff] md:p-8"><h2 className="text-2xl font-bold uppercase text-[#2d3748]">Modération des profils</h2><p className="mt-1 text-sm text-[#566274]">Validez, retirez ou supprimez les profils.</p><div className="mt-6 space-y-3">{profiles.map((profile) => <article key={profile.id} className="grid gap-4 rounded-2xl bg-white p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-[#2d3748]">{profile.name}</h3><Status status={profile.status} /></div><p className="mt-1 text-sm text-[#566274]">{profile.title}</p><p className="mt-2 text-xs text-[#4a5568]">{profile.videoUrl ? "Vidéo renseignée" : "Aucune vidéo"}</p></div><div className="flex flex-wrap gap-2">{profile.status !== "published" && <button onClick={() => void moderate(profile.id, "published")} className="rounded-xl bg-[#dff7e9] px-3 py-2 text-xs font-semibold text-[#17603a]">Publier</button>}{profile.status !== "pending" && <button onClick={() => void moderate(profile.id, "pending")} className="rounded-xl bg-[#fff0d9] px-3 py-2 text-xs font-semibold text-[#8a5208]">En attente</button>}{profile.status !== "removed" && <button onClick={() => void moderate(profile.id, "removed")} className="rounded-xl bg-[#ffe8ef] px-3 py-2 text-xs font-semibold text-[#8a3f5b]">Retirer</button>}{profile.status === "published" && <Link href={`/profils/${profile.id}`} className="rounded-xl bg-[#E8F0F8] px-3 py-2 text-xs font-semibold text-[#273D4F]">Voir</Link>}<button onClick={() => void removeProfile(profile)} disabled={deletingProfileId === profile.id} aria-label={`Supprimer le profil de ${profile.name}`} className="inline-flex items-center gap-1.5 rounded-xl bg-[#8a3f5b] px-3 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60">{deletingProfileId === profile.id ? <Loader2 className="size-3.5 animate-spin" /> : <Trash2 className="size-3.5" />} Supprimer</button></div></article>)}</div></section>
      <aside className="space-y-7 xl:sticky xl:top-6"><section className="rounded-3xl border border-[#A8C5E0] bg-[#F5F9FE] p-6"><h2 className="text-xl font-bold uppercase text-[#2d3748]">Réglages</h2>{settings && <div className="mt-5 space-y-4"><label className="block text-xs font-semibold uppercase tracking-wider text-[#566274]">Seuil de certification<input type="number" min="0" max="100" value={settings.certificationThreshold} onChange={(e) => setSettings({ ...settings, certificationThreshold: Number(e.target.value) })} className="mt-2 h-11 w-full rounded-xl border border-[#1B3A6B]/20 bg-white px-4 text-sm" /></label><label className="block text-xs font-semibold uppercase tracking-wider text-[#566274]">Profils par page<input type="number" min="1" max="20" value={settings.catalogPageSize} onChange={(e) => setSettings({ ...settings, catalogPageSize: Number(e.target.value) })} className="mt-2 h-11 w-full rounded-xl border border-[#1B3A6B]/20 bg-white px-4 text-sm" /></label><button onClick={() => void saveSettings()} className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-[#1B3A6B] px-4 text-sm font-semibold text-white"><Save className="size-4" /> Enregistrer</button></div>}</section></aside>
      </div>}

      {tab === "videos" && <VideoModeration rows={videos} onDecide={decideVideo} />}

      {tab === "questionnaire" &&
      <section className="rounded-3xl border border-[#A8C5E0] bg-[#F5F9FE] p-6 md:p-8"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-[#eee7ff] text-[#65449b]"><FileQuestion className="size-5" /></span><div><h2 className="text-2xl font-bold uppercase text-[#2d3748]">Questionnaire</h2><p className="text-sm text-[#566274]">Libellés et pondérations du calcul JEB.</p></div></div><div className="mt-6 grid gap-3 lg:grid-cols-2">{questions.map((question, index) => <article key={question.id} className="rounded-2xl bg-white p-4"><div className="flex gap-3"><span className="font-mono text-xs font-bold text-[#1B3A6B]">{String(index + 1).padStart(2, "0")}</span><textarea value={question.text} onChange={(e) => setQuestions((rows) => rows.map((row) => row.id === question.id ? { ...row, text: e.target.value } : row))} className="min-h-20 flex-1 resize-none rounded-xl border border-[#1B3A6B]/15 p-3 text-sm outline-none focus:border-[#1B3A6B]" /></div><div className="mt-3 flex items-center justify-between gap-3"><label className="text-xs text-[#566274]">Poids <input type="number" min="1" max="5" value={question.weight} onChange={(e) => setQuestions((rows) => rows.map((row) => row.id === question.id ? { ...row, weight: Number(e.target.value) } : row))} className="ml-2 h-9 w-16 rounded-lg border border-[#1B3A6B]/15 px-2" /></label><div className="flex gap-2"><button onClick={() => void saveQuestion(question)} aria-label="Enregistrer la question" className="flex size-9 items-center justify-center rounded-lg bg-[#D1DEF0] text-[#1B2D3E]"><Save className="size-4" /></button><button onClick={() => void removeQuestion(question.id)} aria-label="Supprimer la question" className="flex size-9 items-center justify-center rounded-lg bg-[#ffe8ef] text-[#8a3f5b]"><Trash2 className="size-4" /></button></div></div></article>)}</div></section>
      }
    </>}
  </main>;
}

...
export function RecruiterDashboard() {
  ...
  return <main className="mx-auto w-full max-w-[1480px] px-5 pb-24 pt-8 md:px-10 md:pt-12 lg:px-6">
    <header className="flex flex-wrap items-end justify-between gap-5 border-b border-[#1B3A6B]/15 pb-7"><div><p className="font-mono text-xs font-semibold uppercase tracking-wider text-[#718096]">Espace recruteur</p><h1 className="mt-3 text-4xl font-extrabold uppercase text-[#2d3748] md:text-5xl">Suivez vos <span className="text-[#1B3A6B]">candidats.</span></h1></div><Link href="/catalogue" className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#1B3A6B] px-5 text-sm font-semibold text-white hover:bg-[#273D4F]"><Search className="size-4" /> Parcourir le catalogue</Link></header>
    <section className="mt-6 grid gap-3 sm:grid-cols-3">{cards.map(({ label, value, icon: Icon, tone }) => <article key={label} className="flex items-center gap-4 rounded-2xl border border-[#1B3A6B]/15 bg-white p-5"><span className={`flex size-10 items-center justify-center rounded-xl ${tone}`}><Icon className="size-5" /></span><div><p className="text-2xl font-extrabold text-[#2d3748]">{value}</p><p className="text-xs text-[#718096]">{label}</p></div></article>)}</section>
    {loading ? <div className="flex justify-center py-24"><Loader2 className="size-7 animate-spin text-[#1B3A6B]" /></div> : <div className="mt-7 grid items-start gap-7 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.6fr)]">
      ...
      <section className="rounded-3xl border border-[#A8C5E0] bg-[#F5F9FE] p-6"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-[#ffe8ef] text-[#8a3f5b]"><Heart className="size-5" /></span><h2 className="text-xl font-bold uppercase text-[#2d3748]">Favoris</h2></div><div className="mt-5 space-y-3">{favorites.length ? favorites.map(({ profile }) => <article key={profile.id} className="flex items-center gap-3 rounded-xl bg-white p-4"><div className="min-w-0 flex-1"><p className="truncate font-bold text-[#2d3748]">{profile.name}</p><p className="truncate text-xs text-[#718096]">{profile.title}</p></div><Link href={`/profils/${profile.id}`} className="text-xs font-semibold text-[#1B3A6B]">Voir</Link><button onClick={() => void removeFavorite(profile.id)} aria-label={`Retirer ${profile.name} des favoris`} className="text-[#8a3f5b]"><Trash2 className="size-4" /></button></article>) : <Empty text="Aucun profil enregistré." />}</div></section>
    </div>}
  </main>;
}

function Empty({ text }: { text: string }) { return <div className="rounded-2xl border border-dashed border-[#1B3A6B]/25 px-5 py-10 text-center text-sm text-[#718096]">{text}</div>; }
